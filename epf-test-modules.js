...
define("epf-test/collections/model_array",
  ...
  function() {
    ...
    var get$ = Ember.get;
    var set$ = Ember.set;
    describe('Ep.ModelArray', function () {
      var array, Post;
      array = null;
      Post = null;
      beforeEach(function () {
        Post = get$(Ep, 'Model').extend({ title: Ep.attr('string') });
        return array = get$(Ep, 'ModelArray').create({ content: [] });
      });
      ...
      return describe('copyTo', function () {
        var dest;
        dest = null;
        beforeEach(function () {
          return dest = get$(Ep, 'ModelArray').create({ content: [] });
        });
        it('should copy objects', function () {
          array.addObjects([
            Post.create({ clientId: '1' }),
            Post.create({ clientId: '2' })
          ]);
          array.copyTo(dest);
    ...
    describe('Ep.PerField', function () {
      ...
      beforeEach(function () {
        ...
        set$(App, 'Post', get$(Ep, 'Model').extend({
          ...
          body: Ep.attr('string'),
          comments: Ep.hasMany(get$(App, 'Comment')),
      ...
      it('keeps ours if only modified in ours', function () {
        ...
        post = session.merge(get$(App, 'Post').create({
          ...
          comments: [get$(App, 'Comment').create({
              id: '3',
              user: get$(Ep, 'LazyModel').create({
                type: get$(App, 'User'),
                id: '2'
              }),
              post: get$(Ep, 'LazyModel').create({
                type: get$(App, 'Post'),
                id: '1'
              })
            })]
...
define("epf-test/model/proxies",
  ...
  function() {
    ...
    var get$ = Ember.get;
    var set$ = Ember.set;
    describe('Ep.LazyModel', function () {
      var App, session;
      App = null;
      session = null;
      beforeEach(function () {
        ...
        set$(this, 'container', new Ember.Container);
        Ep.setupContainer(get$(this, 'container'));
        set$(App, 'Post', get$(Ep, 'Model').extend({ title: Ep.attr('string') }));
        set$(App, 'Comment', get$(Ep, 'Model').extend({
          body: Ep.attr('string'),
          post: Ep.belongsTo(get$(App, 'Post'))
        }));
      ...
      it('loads immediately if already loaded in session', function () {
        var lazyPost;
        lazyPost = session.add(get$(Ep, 'LazyModel').create({
          id: '1',
          type: get$(App, 'Post')
        }));
        session.merge(get$(App, 'Post').create({
        ...
        return get$(expect(get$(lazyPost, 'title')), 'to').eq('wat');
      });
      it('loads belongsTo immediately if already loaded in session', function () {
        var lazyComment, post;
        lazyComment = session.add(get$(Ep, 'LazyModel').create({
          id: '1',
          type: get$(App, 'Comment')
        }));
        post = session.merge(get$(App, 'Post').create({
          id: '1',
          title: 'wat'
        }));
        get$(post, 'comments').addObject(session.merge(get$(App, 'Comment').create({
          id: '1',
          body: 'tmi'
        })));
        return get$(expect(get$(get$(lazyComment, 'post'), 'title')), 'to').eq('wat');
      });
      it('cached load can resolve multiple times', function () {
        var promise;
        session.merge(get$(App, 'Post').create({
          id: '1',
          title: 'wat'
        }));
        promise = session.load(get$(App, 'Post'), 1);
        return promise.then(function (outerPost) {
          return promise.then(function (innerPost) {
            return get$(expect(outerPost), 'to').eq(innerPost);
          });
        });
      });
      return it('lazy model can resolve multiple times', function () {
        var lazyModel;
        session.merge(get$(App, 'Post').create({
          id: '1',
          title: 'wat'
        }));
        lazyModel = get$(Ep, 'LazyModel').create({
          id: '1',
          type: get$(App, 'Post'),
          session: session
        });
        return lazyModel.then(function (outerResolution) {
          return lazyModel.then(function (innerResolution) {
            return get$(expect(outerResolution), 'to').eq(innerResolution);
          });
        });
      });
    });
  });
define("epf-test/model/relationships",
  [],
  function() {
    "use strict";
    // Generated by EmberScript 0.0.14
    ...
    describe('relationships', function () {
      ...
      context('one->many', function () {
        ...
        it('belongsTo updates inverse on delete when initially added as proxy', function () {
          var comment, lazyComment, post;
          post = get$(this, 'session').merge(get$(this, 'session').build('post', {
            id: 1,
            comments: [get$(Ep, 'LazyModel').create({
                type: get$(this, 'Comment'),
                id: 2
              })]
          }));
          lazyComment = get$(get$(post, 'comments'), 'firstObject');
          ...
          set$(lazyComment, 'post', post);
          get$(expect(get$(post, 'comments').toArray()), 'to').eql([get$(lazyComment, 'content')]);
          get$(this, 'session').deleteModel(lazyComment);
          return get$(expect(get$(post, 'comments').toArray()), 'to').eql([]);
        });
    ...
    describe('rest', function () {
      ...
      return describe('deep embedded relationship with leaf referencing a model without an inverse', function () {
        ...
        it('creates new embedded children with reference to new hasMany', function () {
          ...
          campaign = session.merge(get$(this, 'session').build('campaign', { id: 1 }));
      ...
      return context('one->many', function () {
        ...
        it('loads lazily', function () {
          ...
          return session.load('post', 1).then(function (post) {
            ...
            return get$(get$(post, 'comments'), 'firstObject').then(function () {
              get$(expect(get$(adapter, 'h')), 'to').eql([
                'GET:/posts/1',
                'GET:/comments/2'
              ]);
        ...
        it('creates child with lazy reference to parent', function () {
          ...
          return session.flush().then(function () {
            ...
            return get$(get$(get$(expect(get$(post, 'isLoaded')), 'to'), 'be'), 'false');
          });
        });
        ...
        it('deletes child and updates parent', function () {
          ...
          return session.load('post', 1).then(function (post) {
            ...
            return session.flush().then(function () {
              ...
              get$(expect(get$(get$(post, 'comments'), 'length')), 'to').eq(0);
              return get$(expect(get$(post, 'title')), 'to').eq('childless');
            });
          });
        });
        ...
        return context('embedded', function () {
          ...
          it('new parent creates and deletes child before flush', function () {
            ...
            post = session.create(get$(this, 'Post'), { title: 'parent' });
      ...
      context('one->one', function () {
        ...
        it('loads lazily', function () {
          ...
          return session.load(get$(this, 'Post'), 1).then(function (post) {
            ...
            return get$(post, 'user').then(function () {
              get$(expect(get$(adapter, 'h')), 'to').eql([
                'GET:/posts/1',
                'GET:/users/2'
              ]);
    ...
    describe('rest serialization', function () {
      ...
      context('simple model', function () {
        ...
        return describe('serialization', function () {
          ...
          it('handles loaded lazy model', function () {
            var data, lazyPost, post;
            post = get$(this, 'Post').create();
            set$(post, 'id', 1);
            set$(post, 'clientId', '2');
            set$(post, 'title', 'wat');
            set$(post, 'longTitle', 'wat omgawd');
            lazyPost = get$(Ep, 'LazyModel').create({
              id: 1,
              type: get$(this, 'Post')
            });
            lazyPost.resolve(post);
            data = get$(this, 'serializer').serialize(lazyPost);
            return get$(expect(data), 'to').eql({
              client_id: '2',
              id: 1,
              title: 'wat',
...
define("epf-test/session/child_session",
  ...
  function() {
    ...
    var get$ = Ember.get;
    var set$ = Ember.set;
    describe('Ep.ChildSession', function () {
      ...
      beforeEach(function () {
        ...
        set$(Ep, '__container__', get$(this, 'container'));
        Ep.setupContainer(get$(this, 'container'));
        Post = get$(Ep, 'Model').extend({
          title: Ep.attr('string'),
          comments: Ep.hasMany('comment')
        });
        set$(get$(this, 'App'), 'Post', Post);
        Comment = get$(Ep, 'Model').extend({ post: Ep.belongsTo('post') });
      ...
      describe('.load', function () {
        return it('loads immediately if loaded in parent session', function () {
          var post;
          ...
          get$(get$(get$(expect(get$(post, 'isProxy')), 'to'), 'be'), 'true');
          get$(get$(get$(expect(get$(post, 'isLoaded')), 'to'), 'be'), 'true');
  ...
  function() {
    ...
    describe('Ep.Session', function () {
      ...
      describe('.deleteModel', function () {
        ...
        return it('deletes a model via lazy reference', function () {
          var lazyPost, post;
          post = session.merge(session.build('post', { id: 1 }));
          lazyPost = get$(Ep, 'LazyModel').create({
            type: get$(this, 'Post'),
      ...
      describe('.merge', function () {
        it('works with proxy with no corresponding record in session', function () {
          var lazy, merged, post;
          post = get$(this, 'Post').create({
            id: '1',
            title: 'someday'
          });
          lazy = get$(Ep, 'LazyModel').create({ content: post });
          merged = session.merge(lazy);
          return get$(expect(get$(merged, 'id')), 'to').eq('1');
        });
        it('reuses detached model', function () {
          var post;
          post = get$(this, 'Post').create({
        ...
        it('handles merging detached model with hasMany child already in session', function () {
          ...
          post = session.merge(get$(this, 'Post').create({ id: '2' }));
          get$(post, 'comments').addObject(get$(this, 'Comment').create({
            id: '1',
            body: 'obscurity'
          }));
      ...
      context('with orphaned proxy', function () {
        beforeEach(function () {
          set$(this, 'lazyPost', session.merge(get$(Ep, 'LazyModel').create({
            id: '1',
            type: get$(this, 'Post')
          })));
          return set$(this, 'post', session.merge(get$(this, 'Post').create({
            id: '1',
            title: 'this is the title'
          })));
        });
        return it('has actual record in `models`', function () {
          return get$(expect(get$(session, 'models').toArray()), 'to').eql([get$(this, 'post')]);
        });
      });
